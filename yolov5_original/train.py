...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    if rank in [-1, 0]:
        ...
        if not opt.resume:
            ...
            if not opt.noautoanchor:
                check_anchors(dataset, model=model, thr=hyp['anchor_t'], imgsz=imgsz, cfg=opt.cfg)
    ...
    std_indices = np.array(range(dataset.n))
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        model.train()
        # ///////////////////////////////////
        PROGRESS[0] = epoch * 100 / epochs
        # ///////////////////////////////////
        # Update image weights (optional)
        if opt.image_weights:
            # Generate indices
            if rank in [-1, 0]:
                if epoch % opt.noweight_interval == 0:
                    dataset.indices = std_indices
                else:
                    cw = model.class_weights.cpu().numpy() * (1 - maps) ** 2 / nc  # class weights
                    iw = labels_to_image_weights(dataset.labels, nc=nc, class_weights=cw)  # image weights
                    dataset.indices = random.choices(range(dataset.n), weights=iw, k=dataset.n)  # rand weighted idx
            print(dataset.indices[:30])
            # Broadcast if DDP
            if rank != -1:
...
if __name__ == '__main__':
    from utils.yolo_tools import set_names

    parser = argparse.ArgumentParser()
    parser.add_argument('--noweight-interval', type=int, default=2, help='no weights interval')
    ...
    assert opt.workers > 0, "至少需要一个进程，防止"
    with open(opt.data) as f:
        data_dict = yaml.load(f, Loader=yaml.FullLoader)  # data dict
        set_names(data_dict["names"])
    # Set DDP variables
